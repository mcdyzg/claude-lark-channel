...
**Files:**
- Copy to: `~/.claude/plugins/cache/claude-lark-channel/lark-channel/0.1.0/src/`
...
```bash
cd /Users/bytedance/haha/x/agent/claude-about/claude-lark-channel
for dst in ~/.claude/plugins/cache/claude-lark-channel/lark-channel/0.1.0 \
...
for f in src/master/message-parser.ts src/master/feishu-client.ts src/master/bootstrap.ts src/master/index.ts; do
  ...
  CACHE=$(md5 -q "$HOME/.claude/plugins/cache/claude-lark-channel/lark-channel/0.1.0/$f")
  if [ "$LOCAL" = "$CACHE" ]; then
    echo "OK  $f"
  else
    echo "MISMATCH  $f"
