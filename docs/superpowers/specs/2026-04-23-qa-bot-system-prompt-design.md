...
在 `pool.ts#spawnTmux` 构建 `cmd` 时，若 `config.appendSystemPromptFile` 配置且可读且非空，追加 flag：
...
- 配置了 `appendSystemPromptFile` 但文件不存在 / 读不出来 → master logger 输出 `error`，**降级为不加 flag 继续 spawn**，不让 master 因此起不来
