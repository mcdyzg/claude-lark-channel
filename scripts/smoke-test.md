...
> These scenarios are independent; run in any order. Before each scenario, ensure no master is running (`ps aux | grep 'tsx .*src/index' | grep -v grep` returns empty).
...
- [ ] **Non-our-master refusal**: with no master running, find the lock file path (either run and kill a master once, or compute from appId in config.json as `master-<appId>.lock`; assign to `$LOCK` for brevity). Write a fake lock pointing at your shell: `echo $$ > "$LOCK"`. Start a master: `npm run start`. Expect: stderr shows the multi-line `[lark-channel] ✗ cannot acquire lock ...` error recipe, exit code 1, and your shell's PID is **not** killed. Clean up: `rm "$LOCK"`.
- [ ] **Stale-lock self-heal** (not new behavior, regression check): compute `$LOCK` as above. Write a dead PID (`echo 99999 > "$LOCK"` — verify PID 99999 is unused first via `ps -p 99999 || echo "ok, dead"`). Start a master. Expect: normal startup (lock is stolen by existing `lock.ts` logic), no takeover attempt, no `[lark-channel] replacing ...` line in stderr.
- [ ] **Error visibility with debug=false**: set `"debug": false` in config.json. Trigger a known error (e.g. point `appendSystemPromptFile` at a directory like `/tmp`). Start master; DM the bot to trigger a child spawn. Expect: terminal stderr shows the `resolveAppendSystemPromptFile ... is not a regular file` error line (not silent); `~/.claude/channels/lark-channel/logs/debug.log` file is **NOT** created.
- [ ] **Version banner accuracy**: `node -p "require('./package.json').version"` (or `jq -r .version package.json`) to note the value `X.Y.Z`. Start master. Expect: stderr shows `[lark-channel] master v<X.Y.Z> ready (pid=...)` with the exact string (not a hardcoded number).
